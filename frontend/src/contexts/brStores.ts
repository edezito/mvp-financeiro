...
export const useB3Store = create<B3State>((set, get) => ({
  portfolio: null,
  macro: null,
  ipcaHistory: [],
  dividends: {},
  ...
  fetchPortfolio: async () => {
    ...
    try {
      const { data } = await api.get<EnrichedPortfolioResponse>("/api/v1/b3/portfolio");
      set({ portfolio: data, lastUpdated: new Date() });
    } catch (err: any) {
      set({ error: err.response?.data?.detail ?? "Erro ao buscar portfólio B3." });
    } finally {
      set({ loading: false });
    }
  },
  ...
  fetchDividends: async (ticker: string) => {
    try {
      ...
      set((s) => ({ dividends: { ...s.dividends, [ticker.toUpperCase()]: data } }));
    } catch {
      // silencioso
    }
  },
